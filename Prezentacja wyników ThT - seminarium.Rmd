...
pp <- read.csv("amyloid_repeats.csv") %>% gather("day_rep", "intensity", 2:16) %>% 
...
pp_aggr <- group_by(pp, peptides,  day) %>% 
  summarise(intensity = median(intensity))


ggplot(pp, aes(x = day, y = intensity, color = rep)) +
  geom_point() +
...
inner_join(pp_aggr, peptydy, by=c("peptides" = "peptide")) %>% 
...
- nanowłókna curli mają nieskomplikowaną konstrukcję funkcjonalne włókna amyloidalne są bardzo odporne na działanie wysokich temperatur i czynników denatujących
...
Walidacja peptydów przewidzianych przez AmyloGram jako fałszywe pozytywne oraz pozytywne fałszywe.
