...
M_plot <- ggplot(pp, aes(x = day, y = intensity, color = rep)) +
  geom_point() +
...
intensity_plot <- inner_join(pp_aggr, peptydy, by=c("peptides" = "peptide")) %>% 
...
Wybrane zostały 24 peptydy, które miały najniższy wynik. 13 pozytywnych oraz 11 negatywnych.
...
## Wyniki wzorcowych peptydów

```{r, echo=FALSE,message=FALSE,warning=FALSE,fig.width=5,fig.height=2.8}
plot_10
```

## Wartość intensywności dla każdego peptydu 
